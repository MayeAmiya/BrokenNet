...
async function readBigEntry(bigPath: string, wantedPath: string): Promise<Buffer | null> {
  ...
  try {
    ...
    if ((await file.read(header, 0, 16, 0)).bytesRead < 12) return null
    ...
    for (let index = 0; index < fileCount; index++) {
      ...
      if ((await file.read(fixed, 0, 8, cursor)).bytesRead !== 8) return null
      ...
      while (nameBytes.length < 4096) {
        ...
        if ((await file.read(byte, 0, 1, cursor++)).bytesRead !== 1) return null
        if (byte[0] === 0) break
